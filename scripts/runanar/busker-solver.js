...
function main(command) {
  ...
  var theItem = it !== external_kolmafia_namespaceObject.Item.none ? it : items[Math.floor(Math.random() * items.length)];
  if (!args.stock) {
    var _shop$theItem$name;
    var shop = (0,external_kolmafia_namespaceObject.getShop)();
    var quantity = (_shop$theItem$name = shop[theItem.name]) !== null && _shop$theItem$name !== void 0 ? _shop$theItem$name : 0;
    if (quantity < num) {
      (0,external_kolmafia_namespaceObject.putShop)(100, 1, num, theItem);
    } else {
      (0,external_kolmafia_namespaceObject.repriceShop)(100, theItem);
    }
  } else {
    // Stock all items in `items` up to 33
    var _shop = (0,external_kolmafia_namespaceObject.getShop)();
    var _iterator = main_createForOfIteratorHelper(items),
      _step;
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var _shop$i$name;
        var i = _step.value;
        var _quantity = (_shop$i$name = _shop[i.name]) !== null && _shop$i$name !== void 0 ? _shop$i$name : 0;
        if (_quantity < 33) {
          var needed = 33 - _quantity;
          if ((0,external_kolmafia_namespaceObject.retrievePrice)(i) > 500000) throw "".concat(i, " was too expensive, check what went wrong!");
          (0,external_kolmafia_namespaceObject.retrieveItem)(needed, i);
          (0,external_kolmafia_namespaceObject.putShop)(999999999999, 1, needed, i);
        }
      }
    } catch (err) {
      _iterator.e(err);
